...
/**
  * The main entry point for testing StreamDM by running tasks on Spark-Streaming.
  */
object streamDMJob {
  ...
  def main(args: Array[String]) {
    ...
    //run task
    val string = if (paramsArgs.length > 0) paramsArgs.mkString(" ") else "EvaluatePrequential"
